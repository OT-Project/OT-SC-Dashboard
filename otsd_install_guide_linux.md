# OTSD – Central Log Processing System
...
```bash
# Start container
cd OTSD-Docker/single-node/ && docker compose up -d

# Start yarn (mount plugins + run dev server)
cd OTSD-Dashboard && sudo ./scripts/mount-plugins.sh && nvm use && yarn start --no-base-path
```
...
>
> ```
> OTSD-Dashboard-Plugins/plugins/
...
> ```
>
> Cộng thêm `OTSD-Dashboard/src/plugins/*/{public,server}/` cho các plugin built-in của OSD.
>
...
Ví dụ: thêm tính năng "agent grouping" cần sửa cả `OTSD-Dashboard-Plugins` (API backend) lẫn `OTSD-Security-Plugin` (UI).
...
3. Trong PR description, **link chéo** sang PR còn lại:
   ```
   Phần API: OT-Project/OTSD-Dashboard-Plugins#15
   Phần UI:  OT-Project/OTSD-Security-Plugin#8
   ```
...
Làm theo Bước A–C cho cả 4 repo (`OTSD-Dashboard`, `OTSD-Dashboard-Plugins`, `OTSD-Security-Plugin`, `OTSD-Docker`). Sau khi xong, mọi dev đều base trên `dev` để làm việc.
